 ...
 * @return mdl_u32_t
 */
static mdl_u32_t ssmd_stepper ( ssmd_t *p_ssmd )
{
 ...
 * @return mdl_bool_t
 */
static mdl_bool_t ssmd_segment ( ssmd_t *p_ssmd, struct ssmd_block_st *p_temp_block )
{
    mdl_bool_t ret = FALSE;
    struct ssmd_segment_st segment_1;
    struct ssmd_segment_st segment_2;
    struct ssmd_segment_st segment_3;
    mdl_u32_t acc_pluse_count = 0;  /* 加减速脉冲数量 */
    mdl_u32_t segment_pluse_count = 0;
 ...
 * @return mdl_bool_t
 */
static mdl_bool_t ssmd_prepare_segment ( ssmd_t *p_ssmd )
{
    mdl_u32_t segment_pluse_count = 0;
    ...
    if ( p_ssmd->segment_queue.out_method.get_rear_addr ( &p_ssmd->segment_queue, ( void ** ) &p_last_segment ) == TRUE )
    {
        ...
        else // 此处只能是p_last_segment->acceleration < 0
        {
            ...
            struct ssmd_segment_st segment_2;
            struct ssmd_segment_st segment_3;
            mdl_u32_t last_block_speed = 0;
            mdl_u32_t acceleration_count = 0;  /* 加减速脉冲数量 */
            mdl_u32_t deceleration_count = 0; /* 减速脉冲数量 */
 ...
 * @return mdl_bool_t
 */
static mdl_bool_t ssmd_plan_block ( ssmd_t *p_ssmd, struct ssmd_block_st *p_block )
{
    if ( p_ssmd == NULL ) { return FALSE; }
 ...
 * @param is_brake
 */
static void ssmd_brake ( ssmd_t *p_ssmd, mdl_bool_t is_brake )
{
 ...
 * @param is_enabled
 */
static void ssmd_enable ( ssmd_t *p_ssmd, mdl_bool_t is_enable )
{
...
mdl_bool_t mdl_ssmd_init ( ssmd_t *p_ssmd, struct ssmd_config_st config )
{
    ...
    /* 初始化运动段队列，运动规划队列 */
    mdl_ring_queue_create ( &p_ssmd->plan_block_queue, ( mdl_rqueue_data_t * ) p_ssmd->block_buffer, SSMD_PLAN_BLOCK_LENGTH, sizeof ( struct ssmd_block_st ) );
    mdl_ring_queue_create ( &p_ssmd->segment_queue, ( mdl_rqueue_data_t * ) p_ssmd->segment_buffer, SSMD_SEGMENT_LENGTH, sizeof ( struct ssmd_segment_st ) );
